...
class menu:
    ...
    def issue(self):
        ...
        self.aemail = StringVar()  # For email input
        ...
        l3 = Label(self.f1, text='Email : ', font='papyrus 15 bold', bg='black', fg='orange').place(x=50,
                                                                                                    y=200)  # Email label
        e3 = Entry(self.f1, width=25, bd=4, bg='orange', textvariable=self.aemail).place(x=180, y=200)  # Email input

        b1 = Button(self.f1, text='Back', font='Papyrus 10 bold', fg='black', bg='orange', width=10, bd=3,
                    command=self.rm).place(x=50, y=300)
        b2 = Button(self.f1, text='Issue', font='Papyrus 10 bold', fg='black', bg='orange', width=10, bd=3,
                    command=self.issuedbook).place(x=200, y=300)
                ...
                for i in an:
                    if i[1] > 0:
                        try:
                            ...
                            # Sending the email
                            sender_email = "ibadhussain1111@gmail.com"  # Replace with your email
                            sender_password = "qedxcfjgqbitacbx"  # Replace with your email password
                            receiver_email = email  # This should be fetched or entered
                            ...
                            conn.close()
                            messagebox.showinfo("Updated", "Book Issued successfully. Email notification sent.")
...
def Database():
    ...
    if cursor.fetchone() is None:
        cursor.execute("INSERT INTO `login` (username, password) VALUES('Ibad', 'root')")
        conn.commit()
